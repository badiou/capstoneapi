...
assistant_auth_header = {
    'Authorization': myTokens['assistant']
}

director_auth_header = {
    'Authorization': myTokens['director']
}

producer_auth_header = {
    'Authorization': myTokens['producer']
}
...
# ################################################################################################################
    ...
    def test_error_404_get_movies(self):
        res = self.client().get('/movies?page=10000', headers = assistant_auth_header)
        data = json.loads(res.data)
...
# ################################################################################################################
    ...
    def test_error_404_get_actors(self):
        res = self.client().get('/actors?page=10000', headers = assistant_auth_header)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 404)
        self.assertFalse(data['success'])
...
# ################################################################################################################
    ...
    def test_create_new_question(self):
        res=self.client().post('/actors',json= self.new_actor,headers = director_auth_header)
        data=json.loads(res.data)
        self.assertEqual(res.status_code, 200) 
...
# ################################################################################################################
    ...
    def test_create_new_movie(self):
        res=self.client().post('/movies',json= self.new_movie,headers = producer_auth_header)
        data=json.loads(res.data)
        self.assertEqual(res.status_code, 200) 
        self.assertTrue(data['success'],True) 
        self.assertTrue(data['created'])
...
# ################################################################################################################
    ...
    def test_error_404_update_movie(self):
            ...
            res = self.client().patch('/movies/1000', json = updatted_movie, headers = producer_auth_header)
            data = json.loads(res.data)
...
# ################################################################################################################   
    ...
    def test_update_movie(self):
        res = self.client().patch('/movies/1', json = self.new_movie, headers = producer_auth_header)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
...
# ################################################################################################################   
    ...
    def test_update_actor(self):
        res = self.client().patch('/actors/1', json = self.new_actor, headers = producer_auth_header)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertTrue(data['success'])
        self.assertEqual(data['id'], 1)
