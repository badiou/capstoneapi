...
########################################################################################################################################
  ...
  def create_movie(payload):   
    ...
    if not new_release_date:
      abort(400, {"Message":"Request does not contain release_date"})
...
############################################################################################################################################
  ...
  def update_actor(payload,actor_id):
    try:
      body=request.get_json()
      actor_updated=Actor.query.filter(Actor.id==actor_id).one_or_none()
...
############################################################################################################################################
  ...
  def update_movie(payload,movie_id):
    try:
      body=request.get_json()
      movie_updated=Movie.query.filter(Movie.id==movie_id).one_or_none()
      if movie_updated is None:
            abort(404)
      if 'title' in body and 'release_date' in body:
        movie_updated.title=body.get('title',None)
        movie_updated.release_date=body.get('release_date',None)
